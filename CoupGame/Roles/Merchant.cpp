...
namespace CoupG {
    ...
    void Merchant::useAbility() {
        if (this->getCoins() >= 3) {
            this->coins += 1;
            std::cout << this->getName() << " starts their turn with 3 or more coins, receiving 1 extra coin!" <<
                    std::endl;
        }
    ...
    void Merchant::receiveArrestBy(Player &p) {
        ...
        if (this->getCoins() >= 2) {
            ...
            std::cout << this->getName() << " was arrested and paid 2 coins to the bank instead of to another player."
                    << std::endl;
        } else {
            this->setCoins(0);
        }
